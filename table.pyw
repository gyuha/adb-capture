...
class MyTable(QWidget):
    ...
    def __make_table(self):
        ...
        self.table.setEditTriggers(
            QAbstractItemView.NoEditTriggers)  # edit 금지 모드
    ...
    def __btn10_clicked(self):
        self.table.setSelectionBehavior(
            QAbstractItemView.SelectRows)  # row 단위로 선택 가능
    ...
    def __btn18_clicked(self):
        ...
        for idx, sel in enumerate(aa):
            ...
            tmp = "ranage {0} ; row/col Count={1}/{2} ".format(idx, sel.rowCount(), sel.columnCount()) + \
                  "({0},{1}) ~ ({2},{3})".format(sel.topRow(),
                                                 sel.leftColumn(), sel.bottomRow(), sel.rightColumn())
            txt.append(tmp)
    ...
    def __btn19_clicked(self):
        ...
        if aa is not None:
            txt = "row={0}, column={1}, content={2}".format(
                aa.row(), aa.column(), aa.text())
        else:
            txt = "clicked cell = ({0},{1}) ==>None type<==".format(
